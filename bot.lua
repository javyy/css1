	...
	if data.ID == "UpdateNewMessage" then
		...
		if msg.content_.ID == "MessageText" then
			...
			if is_naji(msg) then
				...
				elseif text:match("^(vaysa) (.*)$") then
					local matches = text:match("^vaysa (.*)$")
					if matches == "molhag shodan" then	
						redis:set("botBOT-IDmaxjoin", true)
						redis:set("botBOT-IDoffjoin", true)
						return send(msg.chat_id_, msg.id_, "ok")
					elseif matches == "taed molhag" then	
						redis:set("botBOT-IDmaxlink", true)
						redis:set("botBOT-IDofflink", true)
						return send(msg.chat_id_, msg.id_, "ok")
					elseif matches == "shenasayi molhag" then	
						redis:del("botBOT-IDlink")
						return send(msg.chat_id_, msg.id_, "ok")
					elseif matches == "shomare" then	
						redis:del("botBOT-IDsavecontacts")
						return send(msg.chat_id_, msg.id_, "ok")
					end
				elseif text:match("^(shoro kon be) (.*)$") then
					local matches = text:match("^shoro kon be (.*)$")
					if matches == "molhag shodan" then	
				...
				elseif text:match("^(etaat kon az) (%d+)$") then
					local matches = text:match("%d+")
					if redis:sismember('botBOT-IDadmin', matches) then
						return send(msg.chat_id_, msg.id_, "bashe")
					elseif redis:sismember('botBOT-IDmod', msg.sender_user_id_) then
						return send(msg.chat_id_, msg.id_, "nemishe")
					else
				...
				elseif text:match("^(list) (.*)$") then
					...
					elseif matches == "pasox" then
						local text = ""
						local answers = redis:smembers("botBOT-IDanswerslist")
						for k,v in pairs(answers) do
							text = tostring(text) .. "" .. tostring(k) .. "  " .. tostring(v) .. " : " .. tostring(redis:hget("botBOT-IDanswers", v)) .. ""
						end
				...
				elseif text:match("^(payam haro) (.*)$") then
					local matches = text:match("^payam haro (.*)$")
					if matches == "bebin" then
						redis:set("botBOT-IDmarkread", true)
						return send(msg.chat_id_, msg.id_, "ok")
					elseif matches == "nabin" then
				...
				elseif text:match('^(javab) "(.*)" (.*)') then
					local txt, answer = text:match('^javab "(.*)" (.*)')
				...
				elseif text:match("^(jvb) (.*)$") then
					local matches = text:match("^jvb (.*)$")
					if matches == "bede" then
						redis:set("botBOT-IDautoanswer", true)
						return send(msg.chat_id_, 0, "ok")
					elseif matches == "nade" then
						redis:del("botBOT-IDautoanswer")
						return send(msg.chat_id_, 0, "ok")
					end
				elseif text:match("^(jamkon)$")then
				...
				elseif text:match("^(dar che hali)$") then
					...
					local msgadd = redis:get("botBOT-IDaddmsg") and "okaye" or "okay nis"
					local numadd = redis:get("botBOT-IDaddcontact") and "okaye" or "okay nis"
					local txtadd = redis:get("botBOT-IDaddmsgtext") or  "اد‌دی گلم خصوصی پیام بده"
					local autoanswer = redis:get("botBOT-IDautoanswer") and "okaye" or "okay nis"
					...
					local offjoin = redis:get("botBOT-IDoffjoin") and "okay nis" or "okaye"
					local offlink = redis:get("botBOT-IDofflink") and "okay nis" or "okaye"
					local nlink = redis:get("botBOT-IDlink") and "okaye" or "okay nis"
					local contacts = redis:get("botBOT-IDsavecontacts") and "okaye" or "okay nis"
					local txt = ""..tostring(offjoin).." molhag shodan "..tostring(offlink).." taed link "..tostring(nlink).." shenasayi molhag "..tostring(contacts).." shomare " .. tostring(autoanswer) .." javab "
					return send(msg.chat_id_, 0, txt)
				elseif text:match("^(chegadri)$") or text:match("^(chantayi)$") then
				...
				elseif text:match('^(bego be) "(.*)" (.*)') then
					local id, txt = text:match('^bego be "(.*)" (.*)')
					send(id, 0, txt)
					return send(msg.chat_id_, msg.id_, "goftam")
				elseif text:match("^(benal) (.*)") then
					local matches = text:match("^benal (.*)")
					return send(msg.chat_id_, 0, matches)
				elseif text:match("^(شناسه من)$") then
					return send(msg.chat_id_, msg.id_, "<i>" .. msg.sender_user_id_ .."</i>")
				elseif text:match("^(left bede) (.*)$") then
					local matches = text:match("^left bede (.*)$") 	
					send(msg.chat_id_, msg.id_, 'moshkeli nis')
					tdcli_function ({
					...
					return rem(matches)
				elseif text:match("^(bebar ino) (%d+)$") then
					local matches = text:match("%d+")
					...
					return send(msg.chat_id_, msg.id_, "bordam")
				elseif (text:match("^(hasti)$") and not msg.forward_info_)then
					return tdcli_function({
						ID = "ForwardMessages",
						chat_id_ = msg.chat_id_,
						from_chat_id_ = msg.chat_id_,
				...
				elseif tostring(msg.chat_id_):match("^-") then
					...
					elseif text:match("^(biar hamero)$") then
						tdcli_function({
							ID = "SearchContacts",
							query_ = nil,
							limit_ = 999999999
						...
						return send(msg.chat_id_, msg.id_, "daram miarm")
					end
				end
			end
